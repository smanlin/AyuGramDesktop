...
[[nodiscard]] int ComputeRightBadgeWidth(
	...
	if (badge->role == BadgeRole::User) {
		const auto hasIcon = (badge->identityIcon != IdentityBadgeIcon::None);
		const auto iconWidth = hasIcon
			? st::dialogsPremiumIcon.icon.width()
			: 0;
		const auto iconSkip = (hasIcon && !badge->tag.isEmpty())
		...
		const auto tagWidth = (badge->channel
			...
			? 0
			: badge->tag.maxWidth();
		return iconWidth + iconSkip + tagWidth + boostWidth;
	}
